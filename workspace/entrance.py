...
def execute_single(single_instance):
    ...
    Args:
        single_instance (dict): the instance to be executed (each instance contains all the configs needed)
    """
    # unpacking
    ...
    # assume we are running mpeg emulation
    elif baseline == 'mpeg':
        # unpacking
        video_name = single_instance['video_name']
...
def parameter_sweeping(instances, new_instance, keys):
    ...
    if keys == []: # base case
        ...
        if (isinstance(instances[curr_key], list)): 
            # this case means whether this instance value contains multiple values, if yes
            ...
            # value of the current key is atomic, can executer directly
            # no need for parameter sweeping
...
def execute_all(config_info):
    ...
    """
    # all_instances is a list of dict, each dict is a config set 
